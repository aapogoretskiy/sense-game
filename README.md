...
- Java 17, установленный Docker и Docker Compose.
...
### Загрузка карточек
- **JSON**: отправьте POST запрос на `/api/v1/cards` с телом в формате `UploadCardRequest`.
- **Excel**: отправьте POST запрос на `/api/v1/cards/upload/excel` с файлом формата `.xlsx` в параметре `file`. Столбцы должны называться `Text`, `Category`, `Difficulty`.
- Пример Excel файла расположен по пути `src/main/resources/cards/sample_cards.xlsx` и содержит набор готовых карточек для тестовой загрузки.
